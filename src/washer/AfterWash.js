...
import { Button, CustomItemStatusBar, Loader, updateOrderStatus, ORDER_STATUS_DONE } from '../common';
...
export default class AfterWash extends React.Component {
  constructor(props) {
    super(props);     
  ...
  async jobDone(){
    ...
    if(parseInt(response.data.status) === 1){
      ...
      const resetAction = StackActions.reset({
        index: 0,
        actions: [NavigationActions.navigate({ routeName: 'JobsCards' })],
      });
        ...
        <View style={mainContainer}>
          <TouchableOpacity onPress={() => this.launchCamera()}>
            <LinearGradient
              ...
              >
                <Text style={styles.title}>TAKE PHOTO AFTER WASH</Text>
          ...
          <View style={{padding: 15}}>
            <Button label="Done" onPress={() => this.jobDone()} />
          </View>  
        </View>
      </GSideMenu>
    );
